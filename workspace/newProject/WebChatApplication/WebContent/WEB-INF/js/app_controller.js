    ...
    angular
        ...
        .controller('WebChatAppCtrl', ['$scope', '$rootScope', '$location', 'network_service', '$log', '$http', '$timeout',
            function($scope, $RS, $location, network_service, $log, $http, $timeout) {
                ...
                $RS.$on("do-tab-selection", function() {
                    $scope.location = localStorage.getItem("location");
                    $scope.params = JSON.parse(localStorage.getItem("params")) || {};

                    var fullPath = $scope.location && Object.keys($scope.params).length > 0 ? $scope.location + "?" + JSON.stringify($scope.params) : $scope.location;

                    if (fullPath) {
                        //Specific condition for brandgap due to brandgap health
